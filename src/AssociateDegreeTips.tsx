...
const ExpandableCard = ({ title, icon: Icon, children, defaultOpen = false }: any) => {
  ...
  return (
    <div className="mt-4 bg-white/50 dark:bg-white/5 backdrop-blur-xl rounded-2xl border border-gray-200/50 dark:border-white/10 overflow-hidden transition-all duration-300 hover:shadow-lg hover:shadow-blue-500/5">
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="w-full p-5 flex items-center justify-between text-left group transition-colors"
      >
        <div className="flex items-center gap-4">
          <div className={`p-2 rounded-xl transition-colors ${isOpen ? 'bg-blue-500 text-white' : 'bg-gray-100 dark:bg-white/10 text-gray-500 dark:text-gray-400 group-hover:text-blue-500'}`}>
            {Icon && <Icon size={20} />}
          </div>
        ...
        <ChevronDown
          size={18}
          className={`text-gray-400 transition-transform duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] ${isOpen ? "rotate-180 text-blue-500" : ""}`}
        />
      </button>
      <div
...
const HeroCard = ({ title, subtitle }: { title: string, subtitle: string }) => {
  ...
  return (
    ...
    >
      ...
      >
        <span className="px-5 py-2.5 rounded-full bg-white dark:bg-white/10 backdrop-blur-md text-sm font-bold text-blue-600 dark:text-blue-400 -rotate-6 shadow-lg shadow-blue-500/10 border border-blue-100 dark:border-white/10">GPA 4.0?</span>
      </div>
      <div 
        className="absolute top-20 right-4 md:right-0 hidden md:block transition-transform duration-200 ease-out"
        style={{ transform: `translate(${mousePos.x * -40}px, ${mousePos.y * -40}px)` }}
      >
        <span className="px-5 py-2.5 rounded-full bg-white dark:bg-white/10 backdrop-blur-md text-sm font-bold text-purple-600 dark:text-purple-400 rotate-6 shadow-lg shadow-purple-500/10 border border-purple-100 dark:border-white/10">Non-JUPAS</span>
      </div>
      ...
      >
        <div className="mb-8 p-6 rounded-[2rem] bg-gradient-to-br from-blue-50 to-white dark:from-blue-900/20 dark:to-white/5 shadow-xl shadow-blue-500/10 border border-white/50 dark:border-white/10">
          <GraduationCap size={64} strokeWidth={1.5} className="text-blue-600 dark:text-blue-400" />
        </div>
...
const RetakeAssoSplit = () => {
  ...
  return (
    <div className="flex w-full h-40 rounded-3xl overflow-hidden shadow-sm border border-gray-200 dark:border-white/10">
      ...
      >
        <div className="absolute inset-0 bg-orange-50 dark:bg-orange-900/20 transition-colors duration-300 hover:bg-orange-100 dark:hover:bg-orange-900/30"></div>
        <div className="relative h-full flex flex-col items-center justify-center p-4">
          ...
          <span className="font-bold text-lg text-orange-900 dark:text-orange-100">Retake DSE</span>
          <span className="text-xs font-medium uppercase tracking-wider text-orange-600/70 mt-1">Exam Focus</span>
        </div>
      </div>
      ...
      <div 
        ...
        onMouseLeave={() => setHovered(null)}
      >
        <div className="absolute inset-0 bg-green-50 dark:bg-green-900/20 transition-colors duration-300 hover:bg-green-100 dark:hover:bg-green-900/30"></div>
        <div className="relative h-full flex flex-col items-center justify-center p-4">
          <div className="w-12 h-12 rounded-full bg-green-100 dark:bg-green-500/20 text-green-600 dark:text-green-400 flex items-center justify-center mb-3">
            <CheckCircle2 size={24} />
          </div>
          <span className="font-bold text-lg text-green-900 dark:text-green-100">Associate Degree</span>
          <span className="text-xs font-medium uppercase tracking-wider text-green-600/70 mt-1">New Start</span>
        </div>
      </div>
    </div>
  );
...
const TabbedSelector = () => {
  ...
  const tabs = [
    { id: "ability", label: "Ability", icon: Brain, color: "text-blue-500", bg: "bg-blue-500" },
    { id: "interest", label: "Interest", icon: Star, color: "text-purple-500", bg: "bg-purple-500" },
    { id: "career", label: "Career", icon: Target, color: "text-orange-500", bg: "bg-orange-500" }
  ];
  ...
  return (
    <div className="w-full">
      ...
      <div className="p-8 bg-white dark:bg-white/5 rounded-3xl border border-gray-100 dark:border-white/10 min-h-[120px] flex items-center justify-center text-center shadow-sm">
        <p className="text-lg font-medium text-gray-600 dark:text-gray-300 animate-[fadeIn_0.3s_ease-out]">
  ...
  return (
    <div className="w-full">
      ...
      <div className="h-2 w-full bg-gray-100 dark:bg-white/10 rounded-full mb-8 overflow-hidden">
        <div 
          className="h-full bg-gradient-to-r from-blue-500 to-purple-500 transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)]" 
          ...
          <div 
            ...
            className={`flex items-center gap-4 p-4 rounded-2xl cursor-pointer group transition-all duration-300 border ${
              checkedState[idx]
                ...
                : "bg-white dark:bg-white/5 border-gray-100 dark:border-white/5 hover:border-blue-200 dark:hover:border-blue-800"
            }`}
          >
            <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all duration-300 ${
              ...
              {checkedState[idx] && <Check size={14} className="text-white" />}
            </div>
  ...
  return (
    <div className="flex flex-col items-center w-full py-4">
      <div className="relative w-56 h-56 mb-8 group">
        <div className="absolute inset-0 bg-blue-500/20 blur-3xl rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center z-20">
          ...
          <span className="text-xs text-gray-400 uppercase tracking-widest mt-2 font-semibold">CGPA</span>
        </div>
      </div>
      ...
      <div className="w-full px-8">
        <input 
          ...
          className="w-full h-2 bg-gray-200 dark:bg-white/10 rounded-lg appearance-none cursor-pointer accent-blue-500 hover:accent-blue-400 transition-all"
        />
        <div className="flex justify-between text-xs font-medium text-gray-400 mt-4 font-mono">
          <span>0.0</span>
          <span className="text-blue-500">Drag to simulate</span>
          <span>4.3</span>
        </div>
      </div>
    </div>
    ...
    <div className="group perspective-1000 w-full h-full">
      <div className="relative h-full p-6 rounded-3xl bg-white dark:bg-[#2C2C2E] border border-gray-100 dark:border-white/10 shadow-sm hover:shadow-xl transition-all duration-500 transform hover:-translate-y-2 hover:rotate-x-2 hover:rotate-y-2">
        <div className={`w-14 h-14 rounded-2xl mb-5 flex items-center justify-center transition-transform duration-500 group-hover:scale-110 ${
          type === 'good' ? 'bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400' : 'bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400'
        }`}>
          <Icon size={28} />
        </div>
        <h4 className="text-xl font-bold text-gray-900 dark:text-white mb-3">{title}</h4>
        <p className="text-[15px] text-gray-500 dark:text-gray-400 leading-relaxed">{desc}</p>
      </div>
    </div>
  );
};
...
const FeynmanTimer = () => {
  ...
  return (
    <div className="w-full bg-indigo-50 dark:bg-indigo-900/10 rounded-3xl p-8 flex flex-col items-center text-center border border-indigo-100 dark:border-indigo-500/20">
      <div className="mb-6 relative">
        <div className={`absolute inset-0 bg-indigo-500/20 blur-2xl rounded-full transition-opacity duration-500 ${active ? 'opacity-100' : 'opacity-0'}`}></div>
        <span className="relative text-6xl font-mono font-bold text-indigo-600 dark:text-indigo-400 tabular-nums tracking-tight">
          {Math.floor(timeLeft / 60)}:{(timeLeft % 60).toString().padStart(2, '0')}
        </span>
      </div>
      <p className="text-sm font-medium text-indigo-800 dark:text-indigo-200 mb-8 max-w-xs leading-relaxed">
        Try to explain a concept you just learned in 2 minutes.
      </p>
      <div className="flex gap-4">
        {!active ? (
          <button onClick={() => setActive(true)} className="flex items-center gap-2 px-8 py-3 bg-indigo-600 hover:bg-indigo-500 text-white rounded-full font-semibold transition-all hover:scale-105 active:scale-95 shadow-lg shadow-indigo-500/25">
            <Play size={18} fill="currentColor" /> Start
          </button>
        ) : (
          <button onClick={() => setActive(false)} className="flex items-center gap-2 px-8 py-3 bg-white text-indigo-600 border border-indigo-200 rounded-full font-semibold transition-all hover:bg-indigo-50 active:scale-95">
            <Pause size={18} fill="currentColor" /> Pause
          </button>
        )}
        <button onClick={reset} className="p-3 text-indigo-400 hover:text-indigo-600 dark:hover:text-indigo-300 transition-colors bg-indigo-100/50 dark:bg-indigo-900/30 rounded-full">
          <RotateCcw size={20} />
        </button>
      </div>
    </div>
    ...
    <div className="space-y-8 w-full">
      <div className="relative pt-2">
        <div className="flex justify-between text-sm font-bold mb-3">
          ...
          <span className="text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/30 px-2 py-1 rounded-md">~4.0</span>
        </div>
        <div className="h-4 bg-gray-100 dark:bg-white/10 rounded-full overflow-hidden shadow-inner">
          <div className="h-full bg-gradient-to-r from-blue-400 to-blue-600 w-[95%] animate-[growWidth_1.5s_ease-out] shadow-lg shadow-blue-500/20"></div>
        </div>
        {/* Markers */}
      ...
      <div className="relative pt-2">
        <div className="flex justify-between text-sm font-bold mb-3">
          ...
          <span className="text-green-600 dark:text-green-400 bg-green-50 dark:bg-green-900/30 px-2 py-1 rounded-md">~3.4+</span>
        </div>
        <div className="h-4 bg-gray-100 dark:bg-white/10 rounded-full overflow-hidden shadow-inner">
          <div className="h-full bg-gradient-to-r from-green-400 to-green-600 w-[80%] animate-[growWidth_1.5s_ease-out_0.2s_both] shadow-lg shadow-green-500/20"></div>
        </div>
      </div>
    </div>
  );
...
const LockCard = ({ type, title, subtitle }: { type: "direct" | "conditional", title: string, subtitle: string }) => {
  return (
    <div className={`group p-6 rounded-3xl border transition-all duration-300 hover:scale-[1.02] ${
      type === 'direct' 
        ? 'bg-green-50/50 dark:bg-green-900/10 border-green-100 dark:border-green-900/30 hover:shadow-lg hover:shadow-green-500/10' 
        : 'bg-yellow-50/50 dark:bg-yellow-900/10 border-yellow-100 dark:border-yellow-900/30 hover:shadow-lg hover:shadow-yellow-500/10'
    }`}>
      <div className="flex items-start justify-between mb-5">
        <div className={`p-3.5 rounded-2xl ${type === 'direct' ? 'bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400' : 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-600 dark:text-yellow-400'}`}>
        ...
        <span className={`text-[11px] font-bold px-3 py-1.5 rounded-full uppercase tracking-wider ${
          type === 'direct' ? 'bg-green-100 text-green-700 dark:bg-green-900/50 dark:text-green-300' : 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/50 dark:text-yellow-300'
        }`}>
          {type === 'direct' ? 'SECURE' : 'PENDING'}
        </span>
      </div>
      <h4 className={`text-xl font-bold mb-2 ${type === 'direct' ? 'text-green-900 dark:text-green-100' : 'text-yellow-900 dark:text-yellow-100'}`}>
        {title}
      </h4>
      <p className={`text-sm font-medium ${type === 'direct' ? 'text-green-700 dark:text-green-300' : 'text-yellow-700 dark:text-yellow-300'}`}>
        {subtitle}
      </p>
    </div>
  );
    ...
    sections: [
      {
        ...
        visual: (
          <div className="space-y-6">
            <div className="p-8 bg-white/60 dark:bg-white/5 backdrop-blur-xl rounded-[2rem] border border-white/20 shadow-xl shadow-blue-500/5">
              <h4 className="font-bold text-blue-600 dark:text-blue-400 mb-6 text-center tracking-tight">The Goal: Full Cert</h4>
      ...
      },
      {
        ...
        visual: (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div className="p-6 bg-white/60 dark:bg-white/5 backdrop-blur-xl rounded-[2rem] border border-white/20 shadow-sm hover:shadow-xl hover:shadow-red-500/10 transition-all duration-300 cursor-pointer group hover:-translate-y-1">
              <div className="w-14 h-14 rounded-2xl bg-red-50 dark:bg-red-500/10 text-red-500 flex items-center justify-center mb-5 group-hover:scale-110 transition-transform duration-500">
                <AlertCircle size={28} />
              </div>
              ...
              <div className="mt-6 text-[10px] font-bold text-red-500 bg-red-50 dark:bg-red-500/10 px-3 py-1.5 rounded-full inline-block uppercase tracking-wider">Exam Focus</div>
            </div>
            <div className="p-6 bg-white/60 dark:bg-white/5 backdrop-blur-xl rounded-[2rem] border border-white/20 shadow-sm hover:shadow-xl hover:shadow-green-500/10 transition-all duration-300 cursor-pointer group hover:-translate-y-1">
              <div className="w-14 h-14 rounded-2xl bg-green-50 dark:bg-green-500/10 text-green-500 flex items-center justify-center mb-5 group-hover:scale-110 transition-transform duration-500">
              ...
              <div className="mt-6 text-[10px] font-bold text-green-500 bg-green-50 dark:bg-green-500/10 px-3 py-1.5 rounded-full inline-block uppercase tracking-wider">New Start</div>
            </div>
          </div>
        ),
      ...
      },
      {
        ...
        visual: (
          <div className="bg-white/60 dark:bg-white/5 backdrop-blur-xl rounded-[2rem] p-8 border border-white/20 shadow-xl shadow-blue-500/5">
    ...
    sections: [
      {
        ...
        visual: (
          <div className="space-y-6">
            <div className="p-8 bg-white/60 dark:bg-white/5 backdrop-blur-xl rounded-[2rem] border border-white/20 shadow-xl shadow-blue-500/5">
              <h4 className="font-bold text-blue-600 dark:text-blue-400 mb-6 text-center tracking-tight">目標：Full Cert</h4>
              <div className="flex gap-4 justify-center">
                <FlipNumber value={3} />
                <FlipNumber value={3} />
                <FlipNumber value={2} />
        ...
        visual: (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div className="p-6 bg-white/60 dark:bg-white/5 backdrop-blur-xl rounded-[2rem] border border-white/20 shadow-sm hover:shadow-xl hover:shadow-red-500/10 transition-all duration-300 cursor-pointer group hover:-translate-y-1">
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 font-medium">Retake DSE</p>
              <div className="mt-6 text-[10px] font-bold text-red-500 bg-red-50 dark:bg-red-500/10 px-3 py-1.5 rounded-full inline-block uppercase tracking-wider">專注考試</div>
            </div>
            <div className="p-6 bg-white/60 dark:bg-white/5 backdrop-blur-xl rounded-[2rem] border border-white/20 shadow-sm hover:shadow-xl hover:shadow-green-500/10 transition-all duration-300 cursor-pointer group hover:-translate-y-1">
              <div className="w-14 h-14 rounded-2xl bg-green-50 dark:bg-green-500/10 text-green-500 flex items-center justify-center mb-5 group-hover:scale-110 transition-transform duration-500">
                <CheckCircle2 size={28} />
              ...
              <div className="mt-6 text-[10px] font-bold text-green-500 bg-green-50 dark:bg-green-500/10 px-3 py-1.5 rounded-full inline-block uppercase tracking-wider">全新環境</div>
            </div>
          </div>
        ),
    ...
    <div className="col-span-full w-full">
      <div className="w-full py-12 md:py-24">
              ...
              <div className="pl-8 space-y-8">
                ...
                {t.sections.map((section, index) => (
                  <a 
                    ...
                    className="group flex items-center gap-4 text-sm font-medium text-gray-500 hover:text-blue-600 dark:hover:text-blue-400 transition-colors"
                  >
              ...
              <div key={section.id} id={`section-${section.id}`} className="scroll-mt-32">
                <FadeInSection delay={100}>
                  <div className="flex items-center gap-6 mb-10">
                    <div className="flex items-center justify-center w-16 h-16 rounded-[2rem] bg-white dark:bg-white/5 shadow-lg shadow-gray-200/50 dark:shadow-none border border-gray-100 dark:border-white/10 text-2xl font-bold text-gray-300 dark:text-gray-600">
                      {index + 1}
                    </div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 lg:gap-12">
                    ...
                    <div className="lg:col-span-2 flex flex-col justify-center">
                      <div className="p-8 rounded-[2rem] bg-white/40 dark:bg-white/5 backdrop-blur-md border border-white/20 dark:border-white/10 shadow-sm hover:shadow-md transition-shadow duration-500">
                        <BookOpen className="text-blue-500 mb-6" size={28} />
        ...
        <div className="mt-48 text-center border-t border-gray-100 dark:border-white/5 pt-24">
          <p className="text-2xl md:text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-purple-600 dark:from-blue-400 dark:to-purple-400 max-w-3xl mx-auto leading-tight">
            {lang === "EN" 
