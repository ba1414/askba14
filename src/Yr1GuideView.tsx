...
const Yr1GuideView = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      const sections = YR1_GUIDE.sections.map((_, index) => document.getElementById(`section-${index}`));
  ...
  return (
    <div className="min-h-screen bg-[var(--bg)] relative overflow-hidden">
      ...
      <div className="absolute top-0 left-0 w-full h-[500px] bg-gradient-to-b from-purple-50/50 to-transparent dark:from-purple-900/10 pointer-events-none" />
      <div className="absolute top-[-10%] right-[-5%] w-[500px] h-[500px] bg-purple-200/20 dark:bg-purple-900/20 rounded-full blur-3xl pointer-events-none" />
      <div className="absolute top-[20%] left-[-10%] w-[400px] h-[400px] bg-indigo-200/20 dark:bg-indigo-900/20 rounded-full blur-3xl pointer-events-none" />
        ...
        <div className="fixed inset-0 z-40 lg:hidden bg-black/50 backdrop-blur-sm pt-32" onClick={() => setMobileMenuOpen(false)}>
          <div className="bg-[var(--surface)] mx-4 rounded-2xl shadow-xl overflow-hidden max-h-[70vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="p-4 space-y-1">
              {YR1_GUIDE.sections.map((section, index) => (
                <button
                  key={index}
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 lg:py-12 flex gap-12 items-start">
        ...
        <div className="flex-1 min-w-0 space-y-12">
          ...
          <div className="text-center space-y-8 mb-16 relative">
            <div className="inline-flex items-center justify-center w-24 h-24 rounded-[2rem] bg-gradient-to-br from-purple-100 to-purple-50 dark:from-purple-900/40 dark:to-purple-900/20 text-purple-600 dark:text-purple-400 shadow-lg shadow-purple-500/10 mb-4 ring-1 ring-purple-500/10 animate-fade-in-up">
            ...
            {YR1_GUIDE.sections.map((section, index) => (
              ...
              >
                <div className="flex items-center gap-5 mb-8">
                  <span className="flex-shrink-0 w-12 h-12 rounded-2xl bg-gradient-to-br from-purple-100 to-purple-50 dark:from-purple-900/40 dark:to-purple-900/20 text-purple-600 dark:text-purple-400 flex items-center justify-center text-xl font-bold font-mono shadow-sm ring-1 ring-purple-500/10 group-hover:scale-110 transition-transform duration-300">
                    {index + 1}
                  </span>
                    ...
                    if (paragraph.trim().startsWith('•') || /^\d+\./.test(paragraph.trim())) {
                      ...
                      return (
                        <div key={pIndex} className="flex gap-4 pl-2 group/item hover:translate-x-1 transition-transform duration-200">
                          <div className={`mt-2.5 w-2 h-2 rounded-full flex-shrink-0 shadow-sm ${isNumbered ? 'bg-purple-500 ring-2 ring-purple-200 dark:ring-purple-900' : 'bg-[var(--text-muted)]'}`} />
